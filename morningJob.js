...
exports.morningJob = async () => {
    ...
    const children = await superAgent.get('http://eyesaveserver.herokuapp.com/children');
    const childrenAttendance = await superAgent.get(`http://eyesaveserver.herokuapp.com/childrenAttendance?_date=${date}`);
    const staff = await superAgent.get('http://eyesaveserver.herokuapp.com/staff');
    ...
    for (let child of missingChildren) {
        const parents = await Promise.all(child._escort.map(es => superAgent.get(`http://eyesaveserver.herokuapp.com/escorts/${es}`)))
        ...
        parent && parent.body?._telegramID && sendMessage(parent.body?._telegramID, "Hi, your child is missing. Please update on absence or delay");
    }
    ...
    if (missingChildren.length > 0) {
        for (let i = 0; i < staffTelegramId.length; i++) {
            staffTelegramId[i] && sendMessage(staffTelegramId[i], "Hi, please pay attention: there are missing children");
        }
    }
}
